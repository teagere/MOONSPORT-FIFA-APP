...
export const seedStaff = [
  ["Ava Patel", "Product", "ava@moonsport.local"],
  ["Ben Jacobs", "Trading", "ben@moonsport.local"],
  ["Cara Mokoena", "Marketing", "cara@moonsport.local"],
  ["Daniel Smith", "Finance", "daniel@moonsport.local"],
  ["Emma Naidoo", "People", "emma@moonsport.local"],
  ["Leo Williams", "Tech", "leo@moonsport.local"],
].map(([name, department, email], index) => ({
  id: `staff-${index + 1}`,
  name,
  department,
  email,
